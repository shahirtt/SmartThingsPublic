...
definition(
    ...
    namespace: "SRizk Smart Motion Sensor",
    author: "Shahir Rizk",
    description: "Smart Motion Sensor, return to last state when no motion",
    category: "My Apps",
...
preferences {
	page(name: "pageBasic", title: "When there's activity on sensor", nextPage: "pageAdvanced", uninstall: true) {
		...
		section("And force off if already open and  there’s been no movement for…"){
			input "minutes2", "number", title: "Minutes?"
		}
    ...
    page(name: "pageAdvanced", title: "Advanced options", nextPage: "pageFinal") {
		...
		section("Turn off if already open and then there’s been no movement for.. ?"){
			input "boolforceOff", "bool", defaultValue: true, title: "Turn off if already Open?"
			paragraph "By default, the lights will return to previous state, unless you want to force off if no motion for sometime."
    	}
...
// Number of milliseconds that must have elapsed since motion stop to turn lights off - Forced
def getforceOffThreshold() {
	def retVal = 1000 * 60 * minutes2 - 1000 // 1s before "off" time, just to have some margin
	if (minutes2 == 0) {
    	if (boolDim) {
        	retVal = 29000  // 29 s
        } else {
...
/**
  * If configured to save previous light states, attemps to restore those. If can't find, simply turns off light.
  * Intended to be called when motion Stops.
  */
def turnOffOrRestoreLights() {
    log.debug "Running turnOffOrRestoreLights()..."
    log.trace "state.mode = ${state.mode}"
    if ((!isRunTimeOK() || !isLuxLevelOK() || !isModeOK()) && !boolDontObserve)   {
    	log.trace "Outside specified run time, lux level, or mode. Returning."
		log.debug "Exiting turnOffOrRestoreLights()."
        return
    } else if ((!isRunTimeOK() || !isLuxLevelOK() || !isModeOK()) && boolDontObserve) {
    	log.trace "Outside specified run time, lux level, or mode, but configured not to observe. However, this does not apply to turning light on. Exiting."
		log.debug "Exiting turnOnOrRestoreLights()."
    ...
    if (state.mode != "dim" && isOneRealSwitchOn()) {
    	...
    	if (!isOneSavedSwitchOn() || !boolRemember) {
        	log.trace "No switches were saved as 'on' when motion started or app configured to not remember individual light on/off states. Turning all off and restoring to saved dim level."
            switches.each {
                setDimmerLevel(it, getSavedDimLevelState(it))                
            	it.on()
            }
        	...
        	log.trace "Mode is either 'dim' or 'on,' at leat one switch was saved as on, and app configured to restore states, so restore lights to last known state."
            switches.each {
                def savedLightState = getSavedLightOnOffState(it)
                if (savedLightState != "off") {
                    log.trace "${it} was saved as on. Turning on and resotring previous dimmer level."         
    	...
    	log.trace "State unknown. Turning off all switches."
        switches.off()
    }
    state.mode = "off"    
    log.info "state.mode = ${state.mode}"
...
def turnOnLights() {
	log.debug "Running turnOnLights()..."
    log.info "state.mode = ${state.mode}"
    if ((!isRunTimeOK() || !isLuxLevelOK() || !isModeOK()) && !boolDontObserve)  {
    	log.debug "Outside specified run time, lux level, or mode. Returning."
        return
    ...
    state.mode = "on"
    log.trace "Turned on all lights."
    log.info "state.mode = ${state.mode}"
    log.debug "Exiting turnOffLights()."
}