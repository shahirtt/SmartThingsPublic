 ...
 *
 *  Sonoff Dual Shades Control
 *
 *  Author: Shahir Rizk (Shero)
 *  Date: 2019-03-01
 */
...
metadata {
	definition (name: "Sonoff Dual Curtain Control", namespace: "Shero", author: "Shahir Rizk") {
        capability "Actuator"
		capability "Switch"
		capability "Refresh"
		capability "Sensor"
		...
		attribute "Window Shade", "enum", ["on", "off", "stop"]
    ...
    preferences {
        input name: "fcycle", type: "number", title: "full Interval", description: "Seconds", required: true
        input description: "Once you change values on this page, the corner of the \"configuration\" icon will change orange until all configuration parameters are updated.", title: "Settings", displayDuringSetup: true, type: "paragraph", element: "paragraph"
	...
	tiles (scale: 2){      
		multiAttributeTile(name:"switch", type: "generic", width: 6, height: 4, canChangeIcon: false){
			tileAttribute ("device.switch", key: "PRIMARY_CONTROL") {
				attributeState "Close", label:'${currentValue}',action:"switch.on", backgroundColor:"#ffffff", icon: "st.doors.garage.garage-closed"
				attributeState "Open", label:'Open', action:"switch.off",backgroundColor:"#00a0dc", icon: "st.doors.garage.garage-open"
				//attributeState "Opening", label:'Opening',action:"switch.on", backgroundColor:"#00a0dc", icon: "st.contact.contact.open", nextState:"Closing"
				//attributeState "Closing", label:'${name}', action:"switch.off",  backgroundColor:"#ffffff", icon: "st.contact.contact.closed", nextState:"Closed"
                //attributeState "partially Open", label: 'partially\non', action: "switch.off", icon: "st.doors.garage.garage-closing", backgroundColor: "#D4ACEE", nextState: "closing"
			}
			...
			controlTile("levelSliderControl", "switch.level", "slider",
            height: 1, width: 2) {
		...
		//childDeviceTile("Relay 1", "5CCF7F3C43C2-ep1", height: 2, width: 2, childTileName: "Relay 1")
        //childDeviceTiles("off", "off", height: 2, width: 2, childTileName: "Relay 2")   
        
		standardTile("Open", "Open", width: 2, height: 2, inactiveLabel: false, decoration: "flat") {
            state("on", label: 'Open', action: "switch.on", icon: "st.contact.contact.open")
        }
        ...
        standardTile("Close", "Close", width: 2, height: 2, inactiveLabel: false, decoration: "flat") {
            state("off", label: 'Close', action: "switch.off", icon: "st.contact.contact.closed")
        }
		...
		main(["switch"])
        //details(["switch", "on", "stop", "off", "refresh"])
    }
}
...
private channelNumber(String dni) {
	log.debug dni.split("-ep")[-1] as Integer
    dni.split("-ep")[-1] as Integer
}

private calculateinteval(Long newfcycle) {
	log.debug ("CycleLength: $fcycle")
    log.debug ("NewCycleLength: $newfcycle")
	if (newfcycle != fcycle * 1000 && newfcycle != null) {
    	newfcycle = newfcycle  
        } else {
         newfcycle = fcycle * 1000 as Long
         }
    log.debug ("NewCycleLength: $newfcycle")
}
     ...
     try {
        for (i in 1..2) {
	       addChildDevice("Switch Child Device", "${device.deviceNetworkId}-ep${i}", null,
		      ...
		      isComponent: true, componentName: "ep$i", componentLabel: "Relay $i"])
        }
    } catch (e) {
